...
void setup(void) {
  ...
  Serial.println("Set Strip to white");  
  colorWipe(strip.Color(127, 127, 127), 10); // White
  Serial.println("Ready with setup");  
}
...
void loop(void) {
  if (irrecv.decode(&results)) {
    ...
    if (results.value == 0x20DF18E7) {
      Serial.println("Button8");
      colorWipe(strip.Color(0, 0, 0), 50); // Red
    }
    
    if (results.value == 0x20DF9867) {
